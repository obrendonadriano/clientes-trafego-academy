...
const build = readFileSync(new URL('../n8n/code/build-conversion-payload.js', import.meta.url), 'utf8');
const response = readFileSync(new URL('../n8n/code/read-conversion-response.js', import.meta.url), 'utf8');
...
test('importable workflow embeds tested code and does not retry Meta blindly', () => {
  ...
  assert.equal(flow.nodes.find((n)=>n.name==='Montar payload CAPI').parameters.jsCode,build);
  assert.equal(flow.nodes.find((n)=>n.name==='Ler resposta do Meta').parameters.jsCode,response);
